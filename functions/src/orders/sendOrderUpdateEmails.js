...
export const sendDeliveredEmails = async ({
  ...
  const product = sale?.productBundle ? sale.productBundle[0] : sale.product;
  ...
  await sendEmail({
    ...
    data: {
      ...
      ] : sale.productBundle?.map(productBundle => ({
        name: productBundle.title,
        arrival_date: today,
        order_number: sale.id.slice(0, 6)
      })),
      firstName: seller.firstName
  ...
  const earned = ((totalPrice || product.price) - sale.purchasePriceDetails?.commission - (sale.shippingIncluded ? sale.purchasePriceDetails?.shippingRate : 0)).toFixed(2);
  await sendEmail({
    email: seller.email,
  ...
  await sendEmail({
    ...
    data: {
      name: buyer.firstName + ' ' + buyer.lastName,
      product: !sale.productBundle ? product.title : sale.productBundle[0].title + ` + ${sale.productBundle.length - 1} more`,
      order_number: order.id.slice(0, 6),
      delivery_method: order.shippingCarrier,
    }
  })
